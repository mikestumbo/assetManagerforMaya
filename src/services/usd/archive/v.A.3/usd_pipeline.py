...
USD SCHEMA MAPPING:
ΓöîΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓö¼ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÉ
Γöé Maya Component      Γöé USD Schema                 Γöé
Γö£ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓö╝ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöñ
Γöé Meshes              Γöé UsdGeomMesh                Γöé
Γöé NURBS Curves        Γöé UsdGeomNurbsCurves         Γöé
Γöé Skeleton/Joints     Γöé UsdSkel                    Γöé
Γöé Skin Weights        Γöé UsdSkelBindingAPI          Γöé
Γöé Blendshapes         Γöé UsdSkelBlendShape (2026!)  Γöé
Γöé RenderMan Materials Γöé UsdRi / RenderMan Schemas  Γöé
Γöé Standard Materials  Γöé UsdShade (PreviewSurface)  Γöé
Γöé Constraints         Γöé UsdGeomConstraintTarget    Γöé
Γöé Expressions         Γöé USD variable expressions   Γöé
ΓööΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓö┤ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÿ

USD LAYERS (Maya 2026+):
ΓöîΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓö¼ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÉ
Γöé Layer Type          Γöé Content                    Γöé
Γö£ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓö╝ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöñ
Γöé Root Layer          Γöé References to sublayers    Γöé
Γöé Geometry Layer      Γöé Meshes, Curves, Transforms Γöé
Γöé Skeleton Layer      Γöé Joints, Skinning, Poses    Γöé
Γöé Materials Layer     Γöé Shaders, Textures          Γöé
Γöé Animation Layer     Γöé Keyframes, Constraints     Γöé
Γöé Edit Layer          Γöé Local overrides (import)   Γöé
ΓööΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓö┤ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÿ
...
class ExportOptions:
    ...
    # This dramatically reduces skeleton complexity (e.g., 548 joints ΓåÆ ~40 deformation joints)
    ...
    #   Γö£ΓöÇΓöÇ geometry.usdc (meshes untouched from Maya)
    #   Γö£ΓöÇΓöÇ skeleton.usdc (unified skeleton with 1 Skeleton prim)
    #   ΓööΓöÇΓöÇ animation.usda (editable animation layer)
    ...
    # RenderMan Asset Library (optional ΓÇö user-supplied)
...
class ImportOptions:
    ...
    hybrid_mode: bool = False  # Convert USDΓåÆMaya + import controllers (recommended)
...
class UsdPipeline:
        ...
        for mesh_name, skin_clusters in multi_skin_meshes.items():
            try:
                ...
                if has_blendshapes:
                    ...
                    self.logger.info(
                        f"   [OK] {mesh_name} ΓåÆ {dup_short} (baked geometry, clean skinCluster)"
                    )
                else:
                    self.logger.info(f"   [OK] {mesh_name} ΓåÆ {dup_long.split('|')[-1]} (single skinCluster)")
    ...
    def _get_deformation_skeleton_root(self) -> str | None:
        ...
        if len(roots) == 1:
            ...
            for root in roots:
                self.logger.info(f"   ΓööΓöÇ {root.split('|')[-1]}")
    ...
    def _export_with_mayausd(
        ...
        try:
            ...
            if options.viewport_friendly_skeleton:
                ...
                self.logger.info("≡ƒÄ« ALL NURBS curves excluded from USD - preserved in .rig.mb only")
            else:
            ...
            if options.merge_skeletons:
                ...
                if options.body_skeleton_only:
                    ...
                    self.logger.info(f"   ΓööΓöÇ Total joints in scene: {len(all_joints)}")
                    self.logger.info(f"   ΓööΓöÇ Deformation joints: {len(deform_joints)}")
                    self.logger.info(f"   ΓööΓöÇ Body joints (exported): {len(body_joints)}")
                    self.logger.info(f"   ΓööΓöÇ Facial joints (excluded): {len(facial_excluded)}")
                ...
                if deform_root:
                    self.logger.info(f"   ΓööΓöÇ Deformation root: {deform_root.split('|')[-1]}")
            ...
            # This dramatically reduces skeleton complexity and helps UsdSkelImaging
                ...
                self.logger.info(f"   ΓööΓöÇ Total joints in scene: {len(all_joints)}")
                self.logger.info(f"   ΓööΓöÇ Deformation joints (with parents): {len(deform_joints)}")
                self.logger.info(f"   ΓööΓöÇ Joints excluded: {len(all_joints) - len(deform_joints)}")
            ...
            if options.exclude_xgen_meshes:
                ...
                if xgen_filtered:
                    ...
                    for name in xgen_filtered[:5]:  # Show first 5
                        self.logger.info(f"   ΓööΓöÇ {name}")
                    if len(xgen_filtered) > 5:
                        self.logger.info(f"   ΓööΓöÇ ... and {len(xgen_filtered) - 5} more")
            ...
            if options.viewport_friendly_skeleton and num_skin_clusters > 0:
                ...
                self.logger.info("   ΓööΓöÇ Baking skinned meshes to static geometry...")
                ...
                self.logger.info(f"   ΓööΓöÇ Baked {len(baked_meshes)} skinned meshes to static geometry")
                self.logger.info("   ΓööΓöÇ NURBS curves (controllers) preserved in .rig.mb only")
                self.logger.info("   ΓööΓöÇ Full skinning preserved in .rig.mb backup")
            ...
            export_args = {
                ...
                # NOTE: Keep False here ΓÇö _export_blendshapes_to_usd() handles blendshapes
                ...
                # nurbsCurve  ΓÇö control curve shapes (CVs, IK handles, etc.)
                # nurbsSurface ΓÇö NURBS patch surfaces (eye-lid fitGeo, cluster controls)
            ...
            if options.simplified_skeleton_export and options.viewport_friendly_skeleton:
                ...
                self.logger.info("   ΓööΓöÇ Full rigging preserved in .rig.mb backup")
            ...
            # POST-PROCESS: Structural fix-up ΓÇö must run after material conversion so
            # that the UsdPreviewSurface shaders injected above are visible to the
            # outputs:surface wiring pass.
            # Fixes: outputs:surface wiring, material:binding, SkeletonΓåÆXform re-typing,
    ...
    def _create_layered_usd(
        ...
        Γö£ΓöÇΓöÇ geometry.usdc (meshes, curves, transforms)
        Γö£ΓöÇΓöÇ skeleton.usdc (joints, skinning, blendshapes)
        Γö£ΓöÇΓöÇ materials.usdc (shaders, textures)
        ΓööΓöÇΓöÇ animation.usdc (keyframes, constraints)
        """
    ...
    def _fix_skelroot_scope(self, usd_path: Path) -> bool:
        ...
        try:
            ...
            for sr in skel_roots:
                ...
                self.logger.info(f"   [REFRESH] Re-parenting {old_path} ΓåÆ {new_path}")
    ...
    def _merge_skeleton_prims(self, usd_path: Path) -> bool:
        ...
        try:
            ...
            self.logger.info(f"   [SKEL] {len(all_skeletons)} skeletons ΓåÆ 1 unified skeleton")
    ...
    def _create_animation_layers(
        ...
        try:
            ...
            if len(all_skeletons) == 1:
                ...
                self.logger.info(f"   ΓööΓöÇ Skeleton: {all_skeletons[0].GetPath()}")
                ...
                for pattern in deform_patterns:
                    if pattern in path_str:
                        ...
                        if len(joints) >= 50:  # Deformation skeleton typically has 80+ joints
                            ...
                            self.logger.info(f"      ΓööΓöÇ Joint count: {len(joints)}")
                            break
                if deform_skeleton:
                    break
            ...
            if not deform_skeleton:
                ...
                self.logger.info(f"      ΓööΓöÇ Joint count: {max_joints}")
            ...
            self.logger.info(f"   [SKEL] Skeletons: {len(all_skeletons)} ΓåÆ {len(final_skeletons)}")
    ...
    def _sample_pxr_texture_color(
        ...
        Connection traversal (most reliable first):
          1. connectionInfo(sourceFromDestination) ΓÇö direct single-source lookup
        ...
        try:
            ...
            # ΓöÇΓöÇ Step 1: Collect upstream candidate nodes ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
            candidates: list = []
            ...
            try:
                ...
                if src:
                    # "PxrTexture1.resultRGB" ΓåÆ "PxrTexture1"
            ...
            # ΓöÇΓöÇ Step 2: Find texture file path ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
            FILENAME_ATTRS = (
            ...
            # ΓöÇΓöÇ Step 3: Resolve UDIM / tile tokens ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
            tex_path = re.sub(r'<[Uu][Dd][Ii][Mm]>', '1001', tex_path)
            tex_path = re.sub(r'#{4}', '1001', tex_path)
            tex_path = re.sub(r'%04d', '1001', tex_path)

            # ΓöÇΓöÇ Step 4: Verify file exists, try alt extensions ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
            ...
            # ΓöÇΓöÇ Resolve RenderMan .tex ΓåÆ source image ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
            ...
            # "source.png.tex" ΓÇö stripping the trailing ".tex" gets us back to the
            ...
            if tex_path.lower().endswith('.tex'):
                source_candidate = tex_path[:-4]   # "body.png.tex" ΓåÆ "body.png"
                if os.path.exists(source_candidate):
                    self.logger.info(
                        f"   [TEX-DIAG] RenderMan .tex ΓåÆ source: "
                    ...
                    # ΓöÇΓöÇ One-time dir listing so we can see actual filenames ΓöÇΓöÇ
            ...
            # ΓöÇΓöÇ Attempt 0: user-supplied texture folder ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
            # The user points us at a folder of source PNGs ΓÇö either the flat
            ...
            if _rma_lib and os.path.isdir(_rma_lib):
                # ΓöÇΓöÇ One-shot listing diagnostic (first call only) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
                ...
                # ΓåÆ "Veteran_V008_Base_color_..._1001.png"
                tex_basename_raw = (
                    tex_basename[:-4]
                ...
                def _sample_png(path: str) -> "Optional[Gf.Vec3f]":
                    ...
                    ΓÇö scaling all three channels equally so the brightest channel
                    reaches TARGET_V ΓÇö which preserves the exact hue ratios from
                    ...
                    try:
                        ...
                        with Image.open(path).convert('RGB') as img:
                            ...
                            b = float(arr[:, :, 2].mean())
                            if r + g + b > 0.02:
                                # Proportional lift ΓÇö preserve hue, boost to visibility
                                TARGET_V = 0.45
                                max_ch = max(r, g, b)
                ...
                # Extract .rma folder info early ΓÇö needed by multiple priorities.
                ...
                # ΓöÇΓöÇ Priority 1: flat folder ΓÇö exact filename match ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
                ...
                # ΓöÇΓöÇ Priority 1b: flat folder ΓÇö fuzzy Base_color match ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
                # Handles backups where files are named "{MatName}_Base_color.png"
                # but the .rma stem differs slightly (e.g. LfButn01 ΓåÆ LfButton_01,
                # LfZipr ΓåÆ LfZipper, LwrTeeth ΓåÆ LwrTeeth_3).
                ...
                if 'base_color' in tex_basename_raw.lower():
                    try:
                        ...
                        # (handles LwrTeeth ΓåÆ LwrTeeth_3_Base_color.png)
                        ...
                        # (handles LfButn01 ΓåÆ LfButton_01, LfZipr ΓåÆ LfZipper)
                ...
                # ΓöÇΓöÇ Priority 2: targeted .rma subfolder ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
                ...
                # ΓöÇΓöÇ Priority 3: walk all .rma subfolders as last resort ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
                try:
            ...
            # ΓöÇΓöÇ Attempt 1: Maya MImage ΓÇö reads .tex via RenderMan plugin ΓöÇΓöÇΓöÇΓöÇ
                ...
                with Image.open(tex_path).convert('RGB') as img:
                    ...
                    if r + g + b > 0.02:
                        ...
                        if boosted is not None:
                            self.logger.info(
                                f"   [TEX] {os.path.basename(tex_path)}: "
                                f"({r:.3f}, {g:.3f}, {b:.3f}) via PIL ΓåÆ boosted {boosted}"
                            )
                            return boosted
                        self.logger.info(
                            f"   [TEX] {os.path.basename(tex_path)}: "
                            f"({r:.3f}, {g:.3f}, {b:.3f}) via PIL ΓÇö achromatic, trying rma-scan"
                        )
            except ImportError:
            ...
            # ΓöÇΓöÇ Attempt 2: OpenImageIO (ships with RfM 24+) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
            try:
                import sys as _sys
                ...
                def _oiio_sample(path: str):
                    ...
                    # ΓöÇΓöÇ Strategy A: ImageInput.open ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
                    ...
                    # ΓöÇΓöÇ Strategy B: ImageBuf (handles more format variants) ΓöÇΓöÇ
                ...
                if rgb is not None:
                    ...
                    if r + g + b > 0.02:
                        ...
                        if boosted is not None:
                            self.logger.info(
                                f"   [TEX] {os.path.basename(tex_path)}: "
                                f"({r:.3f}, {g:.3f}, {b:.3f}) via OIIO ΓåÆ boosted {boosted}"
                            )
                            return boosted
                        self.logger.info(
                            f"   [TEX] {os.path.basename(tex_path)}: "
                            f"({r:.3f}, {g:.3f}, {b:.3f}) via OIIO ΓÇö achromatic, trying rma-scan"
                        )
            except ImportError:
            ...
            # ΓöÇΓöÇ Last resort: scan the .rma package dir for any preview / source image ΓöÇΓöÇ
                                    ...
                                    if r + g + b > 0.02:
                                        ...
                                        if boosted is not None:
                                            self.logger.info(
                                                ...
                                                f"({r:.3f}, {g:.3f}, {b:.3f}) ΓåÆ boosted {boosted}"
                                            )
                                            return boosted
                                        self.logger.info(
                                            f"   [TEX] {fname} (rma-scan): "
                                            f"({r:.3f}, {g:.3f}, {b:.3f}) achromatic ΓÇö using name-hash"
                                        )
                            except Exception:
            ...
            self.logger.info(
                f"   [TEX-DIAG] Sampling failed for {os.path.basename(tex_path)} ΓÇö "
    ...
    def _rfm_name_color(self, name: str) -> "Gf.Vec3f":
        ...
        Uses MD5 hash ΓåÆ hue so that every material gets a consistent, spread-out
        ...
        import colorsys
        from pxr import Gf  # type: ignore
        h = int(hashlib.md5(name.encode()).hexdigest()[:8], 16)  # 0ΓÇô4 294 967 295
        hue = h / 4294967296.0   # uniformly spread 0.0ΓÇô1.0
    ...
    def _boost_color_for_display(self, color: "Gf.Vec3f"):
        ...
        if s < 0.08:
            return None  # achromatic ΓÇö name-hash will be more distinctive
        s_out = max(s, 0.62)
    ...
    def _convert_renderman_materials_to_usd_preview(self, usd_path: Path) -> None:
        ...
        try:
            ...
            # ----------------------------------------------------------------
            # Step 1: Build sg_to_color from Maya ΓÇö keyed by shading group name.
            # This is more reliable than meshΓåÆmaterial because skinned mesh
            ...
            if cmds is not None:
                try:
                    # ----------------------------------------------------------
                    # Phase A: Lambert ΓåÆ SG (non-proxy Lamberts only).
                    ...
                    for lambert in (cmds.ls(type="lambert") or []):
                        try:
                            ...
                            # Skip near-black lamberts ΓÇö RfM creates placeholder
                    ...
                    # ----------------------------------------------------------
                    # ΓöÇΓöÇ PHASE B CODE VERSION: v5 ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
                    self.logger.info("   [PHASE-B] v5 ΓÇö scanning RfM SGs for texture colors")
                    ...
                    if _phase_b_rma_lib:
                        ...
                        self.logger.info(
                            "   [PHASE-B] src-lib path: (none) ΓÇö texture colors from asset_100.png"
                        )
                    RFM_SHADER_TYPES = {
                    ...
                    # Build reverse map: Pxr surface shader ΓåÆ its connected SG(s).
                    ...
                    for sg in (cmds.ls(type="shadingEngine") or []):
                        ...
                        try:
                            ...
                            # exist on non-RfM SGs ΓÇö catch the Maya error per-SG)
                            try:
                                rfm_nodes = (
                                    cmds.listConnections(
                            ...
                            # its shader node (e.g. PxrDisneyBsdf1SG ΓåÆ PxrDisneyBsdf1).
                            # In RfM 27 the Pxr shader is NOT wired to .rmanSurface or
                            # .surfaceShader ΓÇö only a lambert placeholder is.  Stripping
                            # the trailing "SG" gives us the shader's short name; we then
                            # look it up in _pxr_short_to_full (handles reference namespaces).
                            if sg.endswith("SG"):
                                derived_short = sg.split(":")[-1][:-2]  # "NS:PxrDisneyBsdf1SG" ΓåÆ "PxrDisneyBsdf1"
                            ...
                            # That Lambert.color ΓåÆ file.fileTextureName is the
                            # actual source PNG ΓÇö readable by PIL, no .tex needed.
                            ...
                            for rfm_node in rfm_nodes:
                                try:
                                    ...
                                    continue
                                # Any Pxr* node is a RenderMan shader ΓÇö mark
                                ...
                                if node_type not in RFM_SHADER_TYPES:
                                    if node_type != "lambert":  # Lambert handled in pre-scan above
                                        self.logger.info(
                                            ...
                                            f"type={node_type!r} ΓÇö not in RFM_SHADER_TYPES, skipping"
                                        )
                                    continue
                                ...
                                try:
                                    ...
                                    if r + g + b > 0.01:
                                        ...
                                        # baseColor is near-zero ΓÇö this shader is
                                        ...
                                        if tex_color is not None:
                                            if sg not in sg_to_color:  # don't overwrite Lambert file result
                                                ...
                                                self.logger.info(
                                                    f"   [TEX] {sg} ΓÇö sampled texture color: {tex_color}"
                                                )
                                            break
            ...
            # Match USD material prim name ΓåÆ Maya SG name using variations.
            # This avoids relying on mesh binding relationships entirely.
            # ----------------------------------------------------------------
            self.logger.info("[LOOKDEV] Scanning USD material prims for nameΓåÆSG matches...")
            usd_mat_count = 0
            ...
            for prim in stage.Traverse():
                ...
                # Also treat near-black Gf.Vec3f as missing for RfM materials ΓÇö
                ...
                if lambert_color is None or (is_rfm_name and is_near_black):
                    ...
                    if is_rfm_name:
                        # Texture sampling failed ΓÇö generate a unique, deterministic
                        ...
                        self.logger.info(
                            f"   [FALLBACK] {usd_mat_name} ΓÇö texture unreadable, "
                            f"using name-hash color: {lambert_color}"
                        )
                    else:
                        continue  # Non-RfM material with no color source ΓÇö skip

                # RfM auto-names SGs after the shader node (e.g. PxrDisneyBsdf1SG).
                if is_rfm_name:
                    self.logger.info(
                        f"   [RFM] {usd_mat_name} ΓÇö writing Phase B color: {lambert_color}"
                    )

                # Check if we already injected a PreviewSurface (re-export).
                # Use path-prefix traversal ΓÇö GetAllDescendants() not available
                ...
                if preview_shader:
                    ...
                    if dc_input and dc_input.HasConnectedSource():
                        ...
                        self.logger.debug(
                            f"   [SKIP] {usd_mat_name} ΓÇö diffuseColor already has texture connection"
                        )
                    else:
                        ...
                        materials_converted += 1
                        self.logger.info(f"   [UPDATE] {usd_mat_name} ΓåÉ {matched_sg} = {lambert_color}")

                    # Ensure material's universal 'surface' output is connected.
                    # The RenderMan exporter only creates ri:surface ΓÇö VP2 needs
                    ...
                    if not surface_out or not surface_out.HasConnectedSource():
                        ...
                        self.logger.debug(
                            f"   [FIX] {usd_mat_name} ΓÇö wired surface output for VP2"
                        )
                else:
                    ...
                    materials_converted += 1
                    self.logger.info(f"   [INJECT] {usd_mat_name} ΓåÉ {matched_sg} = {lambert_color}")
            ...
            if unmatched > 0:
                self.logger.info(f"[LOOKDEV] {unmatched} USD materials had no SG color match ΓÇö check SG keys above")
            if materials_converted > 0:
                ...
                self.logger.info(
                    ...
                    " ΓÇö check SG name samples above"
                )
    ...
    def _fix_exported_usdc(self, usd_path: Path) -> None:
        ...
        1. outputs:surface wiring  ΓÇö mayaUSD with RenderMan shading mode creates
        ...
        2. material:binding  ΓÇö mayaUSD's ``useRegistry`` shading mode skips writing
           material:binding relationships on skinned meshes when the source shaders are
           RenderMan PxrSurface/PxrDisney nodes.  We rebuild the meshΓåÆSGΓåÆMaterial path
           mapping from Maya cmds and write the missing relationships.

        3. GeomSubset binding  ΓÇö face-level material assignments exported as GeomSubset
           prims are named after their shading group; we match the name directly to the
           USD Material prim and write material:binding.

        4. SkeletonΓåÆXform re-typing  ΓÇö mayaUSD types EVERY joint chain as a Skeleton
        ...
        5. SkelAnimation deactivation  ΓÇö SkelAnimation prims paired with the now-Xform'd
           rig-control joints are deactivated so they don't waste evaluation time.

        6. Root-level Mesh deactivation  ΓÇö any Mesh prim that lives outside the default
        ...
        try:
            ...
            # ΓöÇΓöÇ Pass 1: Single traversal \u2014 build all lookup maps ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
            # mat_name_to_path: Material prim name ΓåÆ SdfPath
            # mat_path_to_shader: Material SdfPath ΓåÆ UsdShade.Shader (UsdPreviewSurface)
            mat_name_to_path = {}
            mat_path_to_shader = {}
            for prim in stage.Traverse():
            ...
            # ΓöÇΓöÇ Fix 1: Wire outputs:surface for all materials that are missing it ΓöÇΓöÇ
            surface_wired = 0
            ...
            # ΓöÇΓöÇ Fix 2: Build Maya meshΓåÆSG map (cmds available at export time) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
            mesh_to_sg = {}   # USD Mesh prim transform name ΓåÆ Maya shading group name
            if cmds is not None:
                try:
                    for maya_mesh_shape in (cmds.ls(type='mesh') or []):
                        try:
                            ...
                            # 1. stripNamespaces=True  ΓåÆ prim name = short name only
                            # 2. stripNamespaces=False ΓåÆ colon sanitized to underscore
                            ...
                            pass
                    self.logger.info(f"[FIX] Maya meshΓåÆSG map: {len(mesh_to_sg)} entries")
                except Exception as me:
                    self.logger.warning(f"[FIX] Could not build meshΓåÆSG map: {me}")

            # ΓöÇΓöÇ Fix 3: Write material:binding + identify root-level Mesh prims ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
            ...
            # ΓöÇΓöÇ Fix 4: Deactivate root-level blendshape target Mesh prims + rig-fit helpers
            ...
            #   They are scene-construction helpers, not render geometry ΓÇö deactivate them.
            ...
            # ΓöÇΓöÇ Fix 4b: Set NurbsPatch purpose ΓåÆ guide (belt-and-suspenders for
            #           cases where filterTypes didn't strip them) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
            nurbs_patched = []
            for prim in stage.Traverse():
            ...
            # ΓöÇΓöÇ Fix 5: Find the bind skeleton ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
            ...
            # ΓöÇΓöÇ Fix 6: Re-type FK-rig Skeleton prims ΓåÆ Xform ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
            ...
            self.logger.info(
                f"[FIX] Re-typed {len(skeletons_to_retype)} SkeletonΓåÆXform, "
                f"deactivated {len(skel_anims_to_deactivate)} orphan SkelAnimation prims"
            )

            stage.Save()
            self.logger.info(f"[FIX] Structural fix-up complete ΓåÆ {usd_path.name}")
    ...
    def _create_zip_archive(
        ...
        try:
            ...
            self.logger.info(f"≡ƒù£∩╕Å ZIP archive: {zip_path.name} ({file_size:.1f} MB)")
            return True
    ...
    def import_usd(
        ...
        try:
            ...
            if options.usd_proxy_mode:
                ...
                # This is safe to run on every import ΓÇö it is idempotent and only
                ...
                # root.usda ΓåÆ animation / controllers / materials / skeleton / geometry / base
                ...
                if layered_root:
                    ...
                    self.logger.warning(
                        "[WARNING] Layered stage creation failed ΓÇö "
                ...
                if usd_success and result.usd_meshes > 0:
                    ...
                    # Materials live inside the USD stage ΓÇö rendered by VP2, not Maya Hypershade
                    if result.usd_materials > 0:
                        self.logger.info(
                            f"[LOOKDEV] {result.usd_materials} USD materials rendered via VP2 "
                            f"(UsdPreviewSurface ΓÇö these are USD-native, not Maya Hypershade shaders)"
                        )
                    if options.open_layer_editor:
                        self.logger.info(
                            "[TIP] USD Layer Editor opened ΓÇö author animation as a non-destructive layer (Option B)"
                        )
                    else:
                        self.logger.info(
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    self.logger.info("[SHADING] mayaUsd_ShadingModeImport=useRegistry ΓÇö VP2 will use V3 Lighting API")
                except Exception:
                    pass

                # Sanitize the stem: Maya node names must not contain dots.
                # e.g. "Veteran_Rig.root" ΓåÆ "Veteran_Rig_root"
                ...
                # UI has focus ΓÇö fall back to all model panels so material display
                ...
                try:
                    ...
                    for panel in panels_to_configure:
                        try:
                            ...
                            # useDefaultMaterial=False is the critical flag ΓÇö
    ...
    def _open_usd_layer_editor(self, proxy_shape: str) -> None:
        ...
        This is Option B ΓÇö author animation non-destructively as a USD layer
        ...
        try:
            ...
            try:
                ...
                self.logger.info(
                    "[LAYER] Opened mayaUSD Layer Editor ΓÇö author animation as USD layers (Option B)"
                )
                return
            except Exception:
                pass
    ...
    def _create_animation_edit_layer(self, proxy_shape: str) -> None:
        ...
        try:
            ...
            # ΓöÇΓöÇ Get the live stage from the proxy shape ΓöÇΓöÇ
            stage = None
            ...
            if stage is None:
                self.logger.warning(
                    "[LAYER] Could not access proxy stage ΓÇö "
                    "select the animation sublayer manually in the Layer Editor"
                )
                return

            # ΓöÇΓöÇ Find the animation sublayer and set it as edit target ΓöÇΓöÇ
            root_layer = stage.GetRootLayer()
            for sublayer_path in root_layer.subLayerPaths:
                if "animation" in sublayer_path.lower():
                    ...
                    if anim_layer:
                        ...
                        self.logger.info(
                            "[LAYER] Edit target set to 'animation' sublayer ΓÇö "
                            "keyframes will be authored here"
                        )
                        self.logger.info(
                            "[TIP] Select USD prims in viewport ΓåÆ "
                            "press S to set keyframes on the animation layer"
                        )
                        return

            self.logger.warning(
                "[LAYER] animation sublayer not found ΓÇö "
    ...
    # ΓöÇΓöÇΓöÇ Layered USD Stage Builder ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    def _boost_usd_material_colors(self, usd_path: Path) -> None:
        ...
        Pass 1 ΓÇö UsdPreviewSurface diffuseColor:
        ...
        Pass 2 ΓÇö primvars:displayColor on every Mesh prim:
            ...
            material:binding ΓåÆ UsdPreviewSurface resolution chain,
        ...
        try:
            ...
            hashed = 0
            skipped = 0
            # Maps Material Sdf.Path ΓåÆ final display colour (None = texture-driven)
            mat_path_to_color: dict = {}

            # ΓöÇΓöÇ Pass 1: boost UsdPreviewSurface diffuseColor ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
            for prim in stage.Traverse():
                ...
                # Skip texture-driven inputs ΓÇö never overwrite upstream connections.
                ...
                if boosted_color is not None:
                    ...
                    self.logger.debug(
                        f"   [BOOST] {mat_name}: "
                        f"({raw[0]:.3f}, {raw[1]:.3f}, {raw[2]:.3f}) ΓåÆ {boosted_color}"
                    )
                else:
                    # Achromatic ΓÇö each material gets a unique deterministic hue
                    hash_color = self._rfm_name_color(mat_name)
                    dc_input.Set(hash_color)
                    hashed += 1
                    final_color = hash_color
                    self.logger.debug(
                        f"   [BOOST] {mat_name}: achromatic ΓåÆ name-hash {hash_color}"
                    )

                if mat_path:
                    mat_path_to_color[mat_path] = final_color

            # ΓöÇΓöÇ Pass 2: primvars:displayColor on every Mesh prim ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
            # VP2's UsdSkelImaging adapter renders skinned meshes but does NOT
            # always follow material:binding ΓåÆ UsdPreviewSurface.  Setting
            ...
            #
            # Use ComputeBoundMaterial() ΓÇö the proper USD composition-aware
            ...
            #
            # Fallback guarantee: every mesh gets a displayColor ΓÇö either the
                ...
                try:
                    ...
                    if bound_mat and bound_mat.GetPrim().IsValid():
                        ...
                        if color is None:
                            ...
                            # sentinel) or not in our map ΓÇö name-hash the material.
                            color = self._rfm_name_color(bound_mat.GetPrim().GetName())

                    if color is None:
                        # No material bound at all ΓÇö name-hash the mesh itself
    ...
    def _build_layered_stage(
        ...
        controllerΓåÆjoint mappings are extracted and written into
        ...
        Creates a root .usda whose sublayer stack looks like:
            ...
            root.usda  (this file ΓÇö loaded by the proxy shape)
              Γö£ΓöÇΓöÇ animation.usda      ΓåÉ empty, editable: keyframes
              Γö£ΓöÇΓöÇ controllers.usda    ΓåÉ NURBS curves from .rig.mb (guide)
              Γö£ΓöÇΓöÇ materials.usda      ΓåÉ empty, editable: shader overrides
              Γö£ΓöÇΓöÇ skeleton.usda       ΓåÉ jointΓåÆcontroller metadata
              Γö£ΓöÇΓöÇ geometry.usda       ΓåÉ empty, editable: geo overrides
              ΓööΓöÇΓöÇ <base>.usdc         ΓåÉ original monolithic data (read-only)
        ...
        try:
            ...
            # ΓöÇΓöÇ Extract rig data from .rig.mb if available ΓöÇΓöÇ
            rig_data = None
            if rig_mb_path and rig_mb_path.exists():
                self.logger.info(
            ...
            # ΓöÇΓöÇ Create editorial sublayers ΓöÇΓöÇ
            ...
            for name in layer_names:
                ...
                layer.documentation = (
                    f"USD {name.title()} Override Layer ΓÇö {asset_name}\n"
                ...
                # Populate skeleton sublayer with controllerΓåÆjoint metadata
            ...
            # ΓöÇΓöÇ Create root .usda that composes everything ΓöÇΓöÇ
            ...
            # IMPORTANT: use OverridePrim (or just set defaultPrim metadata) ΓÇö never
            ...
            # Set the defaultPrim metadata on the layer only ΓÇö no prim spec,
            ...
            # ΓöÇΓöÇ Build sublayer stack (strongest on top) ΓöÇΓöÇ
            ...
            ctrl_line = (
                "  controllers.usda  ΓÇö NURBS controllers from .rig.mb\n"
                if has_controllers
                else "  controllers.usda  ΓÇö empty (no .rig.mb available)\n"
            )
            root_layer.documentation = (
                f"USD Layered Rig: {asset_name}\n"
                f"Generated by Asset Manager USD Pipeline\n\n"
                f"Sublayer stack (strongest ΓåÆ weakest):\n"
                f"  animation.usda    ΓÇö keyframes & motion\n"
                f"{ctrl_line}"
                f"  materials.usda    ΓÇö shader overrides\n"
                f"  skeleton.usda     ΓÇö skeleton edits & controller mappings\n"
                f"  geometry.usda     ΓÇö geo overrides\n"
                f"  {base_usd_path.name}  ΓÇö base asset (read-only)\n\n"
            ...
            self.logger.info(
                f"[OK] Layered stage: {root_path.name} ΓåÆ "
    ...
    # ΓöÇΓöÇΓöÇ .rig.mb Controller Extraction ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    def _extract_rig_controllers(
        ...
        Returns a dict:
            {
                ...
                "mappings": {
                    "L_Arm_CTRL": ["L_Arm_JNT"],  # controller ΓåÆ driven joints
                    ...
                },
        ...
        try:
            # ΓöÇΓöÇ Reference the .rig.mb temporarily ΓöÇΓöÇ
            ...
            # forward-compatibility warnings ΓÇö suppressing them keeps the
            # Script Editor clean without hiding any real failures.
            # Suppress deprecated-attribute errors from the nested file load.
            # scriptEditorInfo is a global command ΓÇö no 'edit' flag needed.
            try:
            ...
            try:
                ...
                # ΓöÇΓöÇ Find NURBS curves (controllers) ΓöÇΓöÇ
                all_curves = cmds.ls(
                ...
                # ΓöÇΓöÇ Find controller ΓåÆ joint mappings via constraints ΓöÇΓöÇ
                constraint_types = [
                ...
                self.logger.info(
                    ...
                    f"{len(mappings)} controllerΓåÆjoint mappings"
                )

            finally:
                # ΓöÇΓöÇ Remove the reference ΓöÇΓöÇ
                try:
        ...
        try:
            ...
            for ctrl in controllers:
                ...
                try:
                    ...
                    # Transform ΓÇö only add ops if non-zero
    ...
    def _populate_skeleton_metadata(
        ...
        """
        Write controllerΓåÆjoint mapping metadata into the skeleton sublayer.

        For each Skeleton prim in the base USD, adds custom attributes:
        - assetManager:controllerMap ΓÇö JSON mapping of jointΓåÆcontroller names
        ...
        try:
            ...
            # Invert: controllerΓåÆjoints  ΓåÆ  jointΓåÆcontrollers
            ...
            if skeletons_found:
                self.logger.info(
                    f"[LAYER] Wrote controllerΓåÆjoint mappings "
                    f"for {skeletons_found} skeleton(s) ΓÇö "
        ...
        try:
            ...
            for proxy in proxy_shapes:
                ...
                try:
                    ...
                    if sublayers:
                        self.logger.info(f"≡ƒôÜ USD contains {layer_count} layers:")
            ...
            # Log layer info
            if total_layers > 1:
                self.logger.info(f"≡ƒôÜ Total USD layers loaded: {total_layers}")
    ...
    def _verify_material_colors(self, stage, total_materials: int = 0) -> None:
        ...
        try:
            ...
            # Directly count UsdPreviewSurface shaders ΓÇö more reliable than ComputeSurfaceSource
            preview_shaders = []
            colored_shaders = []
            for prim in stage.Traverse():
                if prim.GetTypeName() != 'Shader':
    ...
    def _import_hybrid(
        ...
        try:
            self.logger.info("[HYBRID] HYBRID WORKFLOW: Converting USD ΓåÆ Maya + NURBS controllers")
            self.logger.info("=" * 70)
            ...
            self.logger.info("   ΓÇó UsdGeomMesh ΓåÆ Maya meshes")
            self.logger.info("   ΓÇó UsdSkel ΓåÆ Maya joints + skinClusters")
            self.logger.info("   ΓÇó RenderMeshVisual ΓåÆ Maya blendShapes")
            self.logger.info("   ΓÇó Materials ΓåÆ RenderMan/Maya shaders")
            ...
            try:
                ...
                self.logger.info(f"≡ƒôÑ Importing USD: {usd_path.name}")
            ...
            self.logger.info(f"   ΓÇó Meshes: {result.meshes_imported}")
            self.logger.info(f"   ΓÇó Joints: {result.joints_imported}")
            self.logger.info(f"   ΓÇó SkinClusters: {result.skin_clusters_imported}")
            self.logger.info(f"   ΓÇó BlendShapes: {result.blendshapes_imported}")
            ...
            # =================================================================
            self.logger.info("")
            self.logger.info("≡ƒÄ« Phase 2: Importing NURBS controllers...")
            ...
            if controllers_imported > 0:
                self.logger.info("")
                self.logger.info("≡ƒöù Phase 3: Connecting controllers to skeleton...")
            ...
            if controllers_imported:
                self.logger.info(f"≡ƒÄ« Controllers: {controllers_imported} NURBS curves")
            self.logger.info("")
    ...
    def _create_usd_skin_bindings(
        ...
        try:
            ...
            self.logger.info("≡ƒöù Checking USD skeleton bindings...")
            ...
            self.logger.info("≡ƒô¥ No existing bindings found - creating new ones...")
            ...
            self.logger.info(f"≡ƒô¥ Created binding layer: {anim_layer.identifier}")
    ...
    def _create_maya_joint_proxies(
        ...
        try:
            ...
            self.logger.info("≡ƒôÑ Importing Maya rig to query skeleton positions...")
            ...
            self.logger.info("≡ƒöù Phase 3.1: Building joint hierarchy...")
            hierarchy_map = {}  # Maps source_joint ΓåÆ parent_source_joint
            ...
            maya_joints = {}  # Maps source_joint ΓåÆ proxy_joint
            ...
            mirrored_proxy_joints = {}  # Maps (original_joint, side) ΓåÆ proxy_joint
            ...
            self.logger.info("≡ƒöù Building proxy joint hierarchy...")
            root_joints = []
            ...
            self.logger.info(f"≡ƒöù Hierarchy: {len(root_joints)} root joint(s)")
    ...
    def _connect_proxy_to_usd_skeleton(
        ...
        """
        try:
            self.logger.info("≡ƒöù Phase 3.2: Connecting proxy joints to USD skeleton...")
            ...
            # Build USD joint mappings (name ΓåÆ path, name ΓåÆ index)
            usd_joint_map = {}
            usd_joint_indices = {}
            ...
            self.logger.info("≡ƒô¥ Using session layer for animation edits")

            # Build joint mapping and create scriptJob connections
            joint_mapping = {}  # proxy_joint ΓåÆ (usd_path, usd_index)
            connections_made = 0
            ...
            self.logger.info(f"≡ƒöù {connections_made} proxy joints driving USD skeleton")
            self.logger.info("[TIP] Animate proxy joints ΓåÆ USD skeleton deforms meshes")
    ...
    def _create_proxy_driver_expressions(
        ...
        Args:
            proxy_shape: The mayaUsdProxyShape node
            joint_mapping: Dict of proxy_joint ΓåÆ (usd_path, usd_index)
        ...
        try:
            ...
            self.logger.info("≡ƒô¥ Creating expression-based skeleton driver...")
    ...
    def _connect_controllers_to_skeleton(
        ...
        try:
            ...
            self.logger.info(f"≡ƒöù Matching {len(controllers)} controllers to {len(converted_joints)} joints...")
            ...
            for controller in controllers:
                ...
                if joint_to_connect:
                    try:
                        ...
                        if constraint:
                            connections_made += 1
                            self.logger.info(f"   [OK] {ctrl_name} ΓåÆ {joint_to_connect.split('|')[-1]}")
                    except Exception as e:
                        self.logger.warning(f"   [WARNING] Could not connect {ctrl_name}: {e}")
                else:
                    self.logger.debug(f"   ΓÅ¡∩╕Å No match for controller: {ctrl_name}")
