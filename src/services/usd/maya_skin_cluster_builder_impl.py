...
class MayaSkinClusterBuilderImpl:
    ...
    def _init__(self):
        ...
        if not MAYA_AVAILABLE:
            logger.error("Maya cmds not available!")
    ...
    def _fix_bind_matrices_from_dagpose(self, skin_cluster: str, joint_names: List[str]) -> None:
        ...
        try:
            ...
            for idx, joint_name in enumerate(influences):
                ...
                except Exception as _joint_error:
                    continue
    ...
    def _force_bind_pose_via_api(self, skin_cluster: str, joint_names: List[str]) -> None:
        ...
        try:
            ...
            if matrices_set == len(influences):
                self.logger.info(f"✅ Successfully set {matrices_set}/{len(influences)} bindPreMatrix values from joint transforms")
            elif matrices_set > 0:
    ...
    def _build_vertex_position_map(
        ...
        try:
            # Get Maya vertex positions in OBJECT SPACE (not world space!)
            # USD positions are in object space, so we need to match in same space
            maya_vertex_count = cmds.polyEvaluate(mesh_name, vertex=True)
            ...
            self.logger.info(f"   Matching {maya_vertex_count} vertices by position...")

            # Build spatial hash of Maya vertices for faster lookup
            # Key: rounded position tuple → Maya vertex index
            # CRITICAL: Use OBJECT SPACE not world space
            maya_position_map = {}
            tolerance = 0.0001  # Position matching tolerance

            for maya_idx in range(maya_vertex_count):
                # Get OBJECT SPACE position (not world space!)
                pos = cmds.xform(f"{mesh_name}.vtx[{maya_idx}]", query=True, objectSpace=True, translation=True)
            ...
            self.logger.info(f"   Matched {len(usd_to_maya_map)}/{len(usd_vertex_positions)} vertices ({match_percentage:.1f}%)")

            if unmatched_count > 0:
                self.logger.warning(f"⚠️ {unmatched_count}/{len(usd_vertex_positions)} vertices could not be matched")
    ...
    def _apply_weights(
        ...
        try:
            self.logger.info(f"Applying weights to {weight_data.vertex_count} vertices...")
            ...
            _ = {joint: idx for idx, joint in enumerate(maya_joints)}
            ...
            for v_idx in range(min(3, weight_data.vertex_count)):
                ...
                non_zero = [(maya_joints[j], w) for j, w in zip(usd_joint_indices, usd_weights) if w > 0.0001 and j < len(maya_joints)]
            ...
            for usd_v_idx in range(weight_data.vertex_count):
                try:
                    # Map USD vertex index to Maya vertex index
                    maya_v_idx = usd_to_maya_vertex_map[usd_v_idx] if usd_to_maya_vertex_map else usd_v_idx

                    # Get USD joint indices and weights for this vertex
                    usd_joint_indices = weight_data.joint_indices[usd_v_idx]
                    usd_weights = weight_data.joint_weights[usd_v_idx]

                    # Build transform-value pairs for Maya
                    # Format: [(joint_name, weight), ...]
                    transform_values = []
                    for joint_idx, weight in zip(usd_joint_indices, usd_weights):
                        if weight > 0.0001:  # Skip near-zero weights
                            if joint_idx < len(maya_joints):
                                joint_name = maya_joints[joint_idx]
                                transform_values.append((joint_name, weight))

                    if not transform_values:
                        vertices_failed += 1
                        if vertices_failed < 10:
                            self.logger.warning(f"USD vertex {usd_v_idx} (Maya {maya_v_idx}) has NO non-zero weights!")
                        continue  # No weights for this vertex

                    # Apply weights using skinPercent
                    vertex_name = f"{mesh_name}.vtx[{maya_v_idx}]"  # Use mapped Maya vertex index
                    cmds.skinPercent(
                        skin_cluster,
                        vertex_name,
                    ...
                    vertices_failed += 1
                    if vertices_failed < 10:  # Limit error spam
                        self.logger.warning(f"Failed to set weights for USD vertex {usd_v_idx}: {vertex_error}")

            success_rate = vertices_processed / weight_data.vertex_count
            self.logger.info(f"Weight application: {vertices_processed}/{weight_data.vertex_count} vertices ({success_rate*100:.1f}%)")
