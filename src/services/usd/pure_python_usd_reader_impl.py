...
class PurePythonUsdReader:
    ...
    def import_usd(self, usd_path: Path, namespace: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            self.logger.info(
                f"[OK] Pure Python import complete: {mesh_count} meshes, {joint_count} joints"
            )
    ...
    def _import_skeletons(self, stage: Any, namespace: str, result: Dict[str, Any]) -> None:
        """
        ...
        for prim in stage.Traverse():
            if prim.IsA(UsdSkel.Skeleton):
                ...
                # Create Maya joints
                maya_joints = self._create_maya_joints(joint_names, bind_transforms, namespace)

                result["joints"].extend(maya_joints)
                result["skeletons"].append(
                    {"usd_path": str(skeleton_path), "maya_joints": maya_joints}
                )

                self.logger.info(f"Created {len(maya_joints)} joints for skeleton {skeleton_path}")

    def _create_maya_joints(
        self, joint_names: List[str], bind_transforms: Optional[Any], namespace: str
    ) -> List[str]:
        """
    ...
    def _import_meshes(self, stage: Any, namespace: str, result: Dict[str, Any]) -> None:
        """
        ...
        for prim in stage.Traverse():
            if prim.IsA(UsdGeom.Mesh):
                ...
                maya_mesh = self._create_maya_mesh(
                    points, face_vertex_counts, face_vertex_indices, str(mesh_path.name), namespace
                )

                if maya_mesh:
                    result["meshes"].append({"usd_path": str(mesh_path), "maya_mesh": maya_mesh})
    ...
    def _create_maya_mesh(
        ...
        try:
            ...
            mesh_transform = cmds.polyCreateFacet(
                point=point_list, constructionHistory=False, name=maya_name
            )[0]
    ...
    def _apply_skinning(self, stage: Any, namespace: str, result: Dict[str, Any]) -> None:
        """
