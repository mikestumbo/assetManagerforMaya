...
class USDExportServiceImpl(IUSDExportService):
    ...
    def export_maya_scene(
        ...
        try:
            ...
            self.logger.info(
                f"🔥 Checking geometry export: export_meshes={options.export_meshes}, "
                f"mesh_count={len(scene_data.meshes)}"
            )
            if options.export_meshes:
                self.logger.info(f"🔥 Starting geometry export for {len(scene_data.meshes)} meshes")
                ...
                self.logger.warning("🔥 GEOMETRY EXPORT SKIPPED - export_meshes is FALSE!")
            ...
            if options.export_nurbs_curves and scene_data.nurbs_curves:
                ...
                self.logger.warning(f"🎯 NURBS EXPORT SKIPPED: export_nurbs_curves={options.export_nurbs_curves}, curves found={len(scene_data.nurbs_curves)}")

            self._update_progress(0.85, "NURBS curves exported")

            # Phase 6: Export rig connections (functional controllers) - INDUSTRY FIRST! (5%)
            if options.export_nurbs_curves and options.export_rig_connections and scene_data.nurbs_curves and (scene_data.rig_connections or scene_data.constraints or scene_data.set_driven_keys):
                ...
                if self._check_cancel():
                    return False
                self.logger.info(f"✨ Exported rig connections: {len(scene_data.rig_connections)} connections, {len(scene_data.constraints)} constraints, {len(scene_data.set_driven_keys)} SDKs")
            ...
            if options.export_skeleton and self.rig_converter:
                self._update_progress(0.9, "Exporting rigging...")
    ...
    def _pre_export_cleanup(self) -> int:
        ...
        try:
            ...
            self.logger.info(f"🔧 PRE-EXPORT CLEANUP: script_path is_file = {os.path.isfile(script_path) if os.path.exists(script_path) else 'N/A'}")
            ...
            if not os.path.exists(script_path):
                ...
                for i in range(5):  # Check up to 5 levels up
                    ...
                    workspace_script = os.path.join(current_dir, "scripts", "HIDE_BLENDSHAPE_TARGETS.py")
                    self.logger.info(f"🔧 PRE-EXPORT CLEANUP: Checking level {i+1}: {workspace_script} (exists: {os.path.exists(workspace_script)})")
                    ...
                    self.logger.warning(f"🔧 PRE-EXPORT CLEANUP: Could not find HIDE_BLENDSHAPE_TARGETS.py, tried 5 levels up from {plugin_dir}")
                    return 0
            ...
            try:
                ...
                self.logger.info(f"🔧 PRE-EXPORT CLEANUP: cleanup_blendshapes_for_export() returned bs_count={bs_count}, dup_count={dup_count}")

                total_hidden = bs_count + dup_count
                self.logger.info(f"✅ Pre-export cleanup complete: {bs_count} blendshapes, {dup_count} duplicates hidden (total: {total_hidden})")
    ...
    def _create_stage(self, output_path: Path, options: USDExportOptions) -> Any:
        ...
        try:
            ...
            if output_path.exists():
                try:
                    output_path.unlink()
                    self.logger.debug(f"Deleted existing file: {output_path}")
                except Exception as e:
    ...
    def _export_geometry(
        ...
        self.logger.info(f"🔥 _export_geometry CALLED with {len(scene_data.meshes)} meshes")
        self.logger.info(f"🔥 export_meshes option: {options.export_meshes}")

        try:
            self.logger.info(f"🔥 Starting mesh loop for {len(scene_data.meshes)} meshes")
            mesh_count = 0
            for mesh_data in scene_data.meshes:
                mesh_count += 1
                self.logger.debug(f"🔥 Processing mesh {mesh_count}/{len(scene_data.meshes)}: {mesh_data.name}")
                if self._check_cancel():
                    return False
            ...
            if meshes_prim.IsValid():
                ...
                self.logger.info(f"🔥 Created {len(mesh_names)} mesh prims: {mesh_names[:10]}{'...' if len(mesh_names) > 10 else ''}")
            else:
                self.logger.error("🔥 /root/meshes prim is NOT VALID after mesh export!")

            self.logger.info(f"🔥 _export_geometry RETURNING TRUE - processed {mesh_count} meshes")
            return True

        except Exception as e:
            self.logger.error(f"🔥 EXCEPTION in _export_geometry: {e}", exc_info=True)
            return False
        ...
        try:
            ...
            for material_data in scene_data.materials:
                ...
                if material_data.is_renderman:
                    # Check if Arnold-USD is available for proper RenderMan conversion
                    if self.usd_service and self.usd_service.arnoldusd_available:
                        # Use Arnold-USD for proper RenderMan material conversion
                        ...
                        if usd_material:
                            self.logger.info(f"Successfully converted RenderMan material {material_name} using Arnold-USD")
                        else:
                            self.logger.warning(f"Failed to convert RenderMan material {material_name} with Arnold-USD")
                    else:
                        # Fall back to Preview Surface conversion for RenderMan materials
                        self.logger.info(f"Converting RenderMan material {material_name} to Preview Surface (Arnold-USD not available)")
                        usd_material = self.material_converter.convert_to_preview_surface(
                            material_name,
                            {
                                'diffuse_color': material_data.diffuse_color or [0.5, 0.5, 0.5],
                                'metallic': material_data.metallic or 0.0,
                                'roughness': material_data.roughness or 0.5
                            },
                            stage
                        )
                else:
                ...
                self.logger.debug(f"Exported material: {material_data.name} ({'RenderMan' if material_data.is_renderman else 'Standard'})")
    ...
    def _export_rig_connections(
        ...
        try:
            ...
            total_connections = len(scene_data.rig_connections) + len(scene_data.constraints) + len(scene_data.set_driven_keys)
    ...
    def export_maya_scene_builtin(self, maya_file, output_path, export_options):
        ...
        try:
            ...
            return ExportResult(success, output_path if success else None, mesh_count, joint_count, skin_cluster_count, curve_count, warnings, error_message)
