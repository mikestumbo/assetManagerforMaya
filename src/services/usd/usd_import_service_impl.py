...
class UsdImportServiceImpl(IUsdImportService):
    ...
    def import_usd_file(
        ...
        try:
            ...
            if options.apply_skin_weights:
                print(f"SUCCESS: Going to call _apply_skin_weights_from_usd with namespace={namespace}")
                self.logger.info(f"HOUDINI: apply_skin_weights=True, calling _apply_skin_weights_from_usd with namespace={namespace}")
                self._apply_skin_weights_from_usd(usd_path, namespace, options, result)
                print(f"SUCCESS: Returned from _apply_skin_weights_from_usd, created {result.skin_clusters_created} skinClusters")
                self.logger.info(f"HOUDINI: Finished _apply_skin_weights_from_usd, created {result.skin_clusters_created} skinClusters")
            else:
    ...
    def _import_with_mayausd(
        ...
        try:
            ...
            # CRITICAL: Enable skeleton/animation import with readAnimData=1
            # This tells mayaUSD to import joint hierarchies, skinning, and animation
            import_options = (
                "readAnimData=1;"  # CRITICAL: Import joints/skeleton/animation
                "shadingMode=useRegistry;"  # Use USD material registry
                # "preferredMaterial=standardSurface;"  # REMOVED: Allow RenderMan materials to import
            )

            self.logger.info(f"Import options: {import_options}")

            # Use mayaUSD import command
            ...
            self.logger.info(f"Imported: {len(result.imported_meshes)} meshes, "
            ...
            if result.imported_joints:
                ...
                if ':' in first_joint:
                    ...
                    if detected_ns != namespace:
                        self.logger.warning(f"Namespace mismatch! Requested '{namespace}' but Maya used '{detected_ns}'")
    ...
    def _apply_skin_weights_from_usd(
        ...
        try:
            ...
            for skel_path in skeletons:
                ...
                self.logger.info(f"SKELETON: Finished skeleton {skel_path}, total skinClusters so far: {result.skin_clusters_created}")
    ...
    def _import_with_pure_python(
        ...
        try:
            ...
            if import_result.get('success'):
                ...
                self.logger.info(f"Pure Python import: {len(result.imported_meshes)} meshes, {len(result.imported_joints)} joints")
                return namespace
