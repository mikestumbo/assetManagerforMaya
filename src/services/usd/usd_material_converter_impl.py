...
class USDMaterialConverterImpl(IUSDMaterialConverter):
    ...
    def convert_renderman_material(
        ...
        try:
            # Check if Arnold-USD is available for proper conversion
            arnold_available = usd_service and getattr(usd_service, 'arnoldusd_available', False)

            if arnold_available:
                # Use Arnold-USD for proper RenderMan conversion
                return self._convert_renderman_with_arnold(
                    material_name, renderman_params, usd_stage
                )
            else:
                # Fall back to manual conversion
                self.logger.info(f"Using manual RenderMan conversion for {material_name} (Arnold-USD not available)")
                return self._convert_renderman_manual(
                    material_name, renderman_params, usd_stage
                )
    ...
    def _convert_renderman_with_arnold(
        self,
        material_name: str,
        renderman_params: Dict[str, Any],
        usd_stage: Any
        ...
        """
        Convert RenderMan material using Arnold-USD plugin
        ...
        """
        try:
            # Import Arnold-USD modules
            try:
                from arnold import (  # type: ignore
                    AiNode,
                    AiNodeSetRGB, AiNodeSetInt, AiNodeSetFlt, AiNodeSetStr
                )
                _ = True
            except ImportError:
                self.logger.warning("Arnold Python API not available, falling back to manual conversion")
                return self._convert_renderman_manual(material_name, renderman_params, usd_stage)
            ...
            # Use Arnold to create RenderMan shader node
            shader_type = self._determine_renderman_shader_type(material_name, renderman_params)

            # Create Arnold node for RenderMan shader
            arnold_node = AiNode(shader_type)
            if not arnold_node:
                self.logger.error(f"Failed to create Arnold node for {shader_type}")
                return self._convert_renderman_manual(material_name, renderman_params, usd_stage)

            # Set parameters on Arnold node
            for param_name, param_value in renderman_params.items():
                try:
                    # Convert parameter types as needed
                    if isinstance(param_value, list) and len(param_value) == 3:
                        # RGB/color parameter
                        AiNodeSetRGB(arnold_node, param_name, param_value[0], param_value[1], param_value[2])
                    elif isinstance(param_value, (int, float)):
                        # Numeric parameter
                        if isinstance(param_value, int):
                            AiNodeSetInt(arnold_node, param_name, param_value)
                        else:
                            AiNodeSetFlt(arnold_node, param_name, param_value)
                    elif isinstance(param_value, str):
                        # String parameter
                        AiNodeSetStr(arnold_node, param_name, param_value)
                    # Skip unsupported parameter types

                except Exception as e:
                    self.logger.debug(f"Failed to set Arnold parameter {param_name}: {e}")
                    continue

            # Create USD shader from Arnold node
            shader = UsdShade.Shader.Define(usd_stage, f"{material_path}/Shader")
            if not shader or not shader.GetPrim().IsValid():
                self.logger.error(f"Failed to create USD shader prim for {material_name}")
                return None

            # Set shader ID for RenderMan
            shader.CreateIdAttr(f"ri:{shader_type}")

            # Connect Arnold parameters to USD shader
            # This is a simplified version - full implementation would need more complex mapping

            # Create surface output connection
            surface_output = usd_material.CreateSurfaceOutput()
            shader_output = shader.CreateOutput("out", Sdf.ValueTypeNames.Token)
            surface_output.ConnectToSource(shader_output)

            self.logger.info(f"Created RenderMan USD material with Arnold-USD: {material_name} ({shader_type})")
            return usd_material

        except Exception as e:
            self.logger.error(f"Arnold-USD conversion failed for {material_name}, falling back to manual: {e}")
            return self._convert_renderman_manual(material_name, renderman_params, usd_stage)
