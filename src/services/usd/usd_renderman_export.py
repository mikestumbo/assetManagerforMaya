...
def create_unified_hierarchy():
    ...
    dup_joints = cmds.listRelatives(skel_long, allDescendents=True, type='joint', fullPath=True) or []
    dup_joints.insert(0, skel_long)
    print(f'Total joints: {len(dup_joints)}')

    new_skin = cmds.skinCluster(dup_joints, mesh_long, toSelectedBones=True, name='USD_skinCluster')[0]
