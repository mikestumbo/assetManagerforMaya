...
def test_service_container_initialization() -> Tuple[bool, str]:
    ...
    try:
        ...
        if len(registered_services) >= 2:  # Should have at least thumbnail and repository services
            return (
                True,
                f"EMSA container initialized with {len(registered_services)} services: {list(registered_services.keys())}",
            )
        else:
...
def test_asset_repository_integration() -> Tuple[bool, str]:
    ...
    try:
        ...
        if repo:
            ...
            has_find_method = hasattr(repo, "find_all") or hasattr(
                repo, "get_assets_from_directory"
            )
            return (
                True,
                f"Repository service active: {repo_type}, methods available: {has_find_method}",
            )
        else:
...
def test_thumbnail_service_integration() -> Tuple[bool, str]:
    ...
    try:
        ...
        if thumbnail_service:
            ...
            return (
                True,
                f"Thumbnail service active: {service_type}, generate: {has_generate}, cache: {has_cache}",
            )
        else:
...
def test_maya_workspace_integration() -> Tuple[bool, str]:
    ...
    try:
        ...
        return (
            True,
            f"Workspace: {os.path.basename(current_project) if current_project else 'None'}, Scene: {os.path.basename(scene_name)}, Accessible: {workspace_accessible}",
        )
...
def test_installation_files() -> Tuple[bool, str]:
    ...
    try:
        ...
        icon_files = ["assetManager_icon.png", "assetManager_icon2.png"]
        ...
        for location_name, install_dir in install_locations:
            ...
            if len(present_core) >= 2:  # At least 2 core files present
                ...
                if len(missing_core) == 0:
                    return (
                        True,
                        f"All core files present in {location_name}: {install_dir} | Found: {found_files}/{total_files} files",
                    )
                else:
                    return (
                        False,
                        f"Missing core files in {location_name}: {', '.join(missing_core)} | Icons: {len(present_icons)}/{len(icon_files)}",
                    )
        ...
        return (
            False,
            f"Installation validation failed. Checked locations: {'; '.join(locations_info) if locations_info else 'None found'}",
        )
